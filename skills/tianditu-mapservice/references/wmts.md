...
| 现象 | 原因 | 解决 |
|------|------|------|
| 返回 301012 权限类型错误 | Key 类型与调用场景不匹配（**由 User-Agent 判定**） | 服务端调用（curl/QGIS）用「服务端」Key；浏览器用「浏览器端」Key |
...
|---------|-----------|-------------|
| curl 默认 UA（服务端） | ✅ 200 | ❌ 403 / 301012 |
| 浏览器 UA（Chrome） | ❌ 403 | ✅ 200 |

> 实测结论：瓦片服务与 Web 服务 API 校验机制一致——**服务端调用必须服务端 Key，浏览器调用必须浏览器端 Key**，由请求的 User-Agent 判定。浏览器端 Key 在浏览器 UA 下可调通全部接口（含 Web 服务 API）。
